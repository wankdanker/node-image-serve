...
ParsedName.OPTIONS = {
	...
	size: /-size:(([0-9]+)x([0-9]+))/,
	crop: /-(cropped|crop):([0-9]*\.?[0-9]+)x([0-9]*\.?[0-9]+)~([0-9]*\.?[0-9]+),([0-9]*\.?[0-9]+)/,
	trim: /-trimmed|-trim/,
	...
	background: /-background:([a-zA-Z]+)/,
	square: /-square|-squared/,
	nosquare: /-nosquare|-notsquared/,
	sharpen: /-sharpen:([0-9]+)/
};
...
function ParsedName(name) {
	...
	this.background = (ParsedName.OPTIONS.background.exec(name) || "")[1] || null;
	this.sharpen = (ParsedName.OPTIONS.sharpen.exec(name) || "")[1] || null;
	...
	this.square = ParsedName.OPTIONS.square.test(name)
		? true:
		ParsedName.OPTIONS.nosquare.test(name) ?
		false :
		null;
